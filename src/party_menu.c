...
static void CB2_ReturnToPartyMenuUsingGobstopper(void);
...
void ItemUseCB_Gobstopper(u8 taskId, TaskFunc task);
...
static void Task_ClosePartyMenuAndSetCB2(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        else if (VarGet(VAR_USING_KEYITEM) == 1)
        {
...
{
    return CheckBagHasItem(MoveToHM(move), 1);
}

u16 MoveToFlag(u16 move)
{
    u16 id;
    switch (move)
    {
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    // Add non-HM field moves to action list
    // Add field moves to action list
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        for (j = 8; j != FIELD_MOVES_COUNT; j++)
        {
...
static void CB2_ReturnToPartyMenuWhileLearningMove(void)
{
    ...
    else if (ItemId_GetFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_Gobstopper && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
        InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_USE_ITEM, TRUE, PARTY_MSG_NONE, Task_ReturnToPartyMenuWhileLearningMove, gPartyMenu.exitCallback);
    else
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    if (targetSpecies != SPECIES_NONE)
    {
        ...
        else if (gSpecialVar_ItemId == ITEM_GOBSTOPPER && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
            gCB2_AfterEvolution = CB2_ReturnToPartyMenuUsingGobstopper;
        else
            gCB2_AfterEvolution = gPartyMenu.exitCallback;
        BeginEvolutionScene(mon, targetSpecies, evoModeNormal, gPartyMenu.slotId);
        DestroyTask(taskId);
